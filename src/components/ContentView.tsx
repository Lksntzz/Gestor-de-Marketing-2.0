...
export const ContentView: React.FC<ContentViewProps> = ({
  ...
  const refreshEditorialItems = async () => {
    ...
    try {
      ...
      setEditorialItems(Array.isArray(items) ? items : []);
    } catch (error) {
  ...
  const openPlanningForScript = (script: CreativeScript) => {
    const contentType = clean(script.format);
    const platform = clean(script.platform);
    const objective = clean(script.objective);
    if (!contentType || !platform || !objective) {
      setNotice({
        type: "warning",
        text: "Este conteúdo salvo não possui formato, plataforma e objetivo suficientes para entrar no calendário.",
      });
      setLibraryOpen(false);
      return;
    }
  ...
  const libraryAction = (entry: CreativeLibraryEntry) => {
    ...
    if (entry.status === "approved" && entry.script) {
      return (
        <button
          type="button"
          onClick={() => openPlanningForScript(entry.script!)}
          className="px-3 py-2 rounded-lg border border-indigo-500/25 bg-indigo-500/10 text-indigo-300 text-[11px] font-bold flex items-center gap-1.5"
        >
    ...
    if (entry.idea) {
      return (
        <button
          type="button"
          onClick={() => resumeIdea(entry.idea!)}
          className="px-3 py-2 rounded-lg border border-pink-500/25 bg-pink-500/10 text-pink-300 text-[11px] font-bold flex items-center gap-1.5"
        >
          <Film className="w-3.5 h-3.5" />
          {entry.status === "development" ? "Retomar" : "Desenvolver"}
        </button>
      );
    }
    return null;
          ...
          <button
            ...
            className={`px-3 py-2 rounded-xl border text-xs font-bold flex items-center gap-2 ${
              libraryOpen
                ...
                : "border-outline-border text-text-secondary hover:text-text-primary"
            }`}
          >
            <BookOpen className="w-4 h-4" />
            Biblioteca
            {libraryCounts.total > 0 && (
              <span className="min-w-5 h-5 px-1 rounded-full bg-background border border-outline-border flex items-center justify-center text-[10px]">
                {libraryCounts.total}
              </span>
            )}
          </button>
          ...
          <section className="mb-5 bg-surface-card border border-outline-border p-5 rounded-2xl">
            <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4 border-b border-outline-border pb-4">
              <div>
                ...
                <p className="mt-1 text-[11px] text-text-secondary max-w-2xl">
                  Uma visão derivada do que já está salvo. Não existe um banco paralelo: Ideias vêm do estado local, conteúdos aprovados dos roteiros e Planejado do calendário editorial.
                </p>
              </div>
            ...
            {libraryEntries.length === 0 ? (
              <div className="py-10 text-center text-text-secondary">
                ...
                <p className="mt-3 text-sm font-bold text-text-primary">A Biblioteca ainda está vazia</p>
                <p className="mt-1 text-xs">Salve uma ideia para iniciar o fluxo operacional.</p>
              </div>
            ) : (
                    ...
                    <article key={entry.id} className="rounded-xl border border-outline-border bg-background/35 p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <span className={`inline-flex px-2 py-1 rounded-full border text-[9px] font-black uppercase tracking-wide ${status.className}`}>
                            {status.label}
                          </span>
                          <h3 className="mt-2 font-bold text-sm truncate">{entry.title}</h3>
                          <p className="mt-1 text-[10px] text-text-secondary">
                            {[entry.script?.format || entry.idea?.format, entry.script?.platform || entry.idea?.channel, entry.script?.objective || entry.idea?.objective]
                              .filter(Boolean)
                              .join(" • ") || "Sem metadados adicionais"}
                          </p>
                          {entry.plannedItem && (
                            <p className="mt-2 text-[10px] text-indigo-300">
                              {entry.plannedItem.scheduledDate}
                              {entry.plannedItem.scheduledTime ? ` às ${entry.plannedItem.scheduledTime}` : ""}
                              {entry.plannedItem.platform ? ` • ${entry.plannedItem.platform}` : ""}
                            </p>
                          )}
                        </div>
            ...
            <section className="bg-surface-card border border-outline-border p-5 rounded-2xl">
              ...
              <div className="mt-5 grid grid-cols-1 md:grid-cols-2 gap-4">
                <label className="md:col-span-2">
                  ...
                  <input value={briefing.objective} onChange={(event) => updateBriefing("objective", event.target.value)} className="w-full bg-background border border-outline-border rounded-lg px-3 py-2 text-sm outline-none focus:border-pink-500/60" placeholder="Ex: apresentar uma nova linha de produto" />
                </label>
                <label>
                  <span className="block text-[10px] uppercase text-text-secondary font-bold mb-1">Formato *</span>
                  <input value={briefing.format} onChange={(event) => updateBriefing("format", event.target.value)} className="w-full bg-background border border-outline-border rounded-lg px-3 py-2 text-sm outline-none focus:border-pink-500/60" placeholder="Ex: Reel, carrossel, artigo" />
                </label>
                <label>
                  <span className="block text-[10px] uppercase text-text-secondary font-bold mb-1">Canal *</span>
                  <input value={briefing.channel} onChange={(event) => updateBriefing("channel", event.target.value)} className="w-full bg-background border border-outline-border rounded-lg px-3 py-2 text-sm outline-none focus:border-pink-500/60" placeholder="Ex: Instagram, TikTok, LinkedIn" />
                </label>
                <label className="md:col-span-2">
                  <span className="block text-[10px] uppercase text-text-secondary font-bold mb-1">Tema opcional</span>
                  <input value={briefing.theme} onChange={(event) => updateBriefing("theme", event.target.value)} className="w-full bg-background border border-outline-border rounded-lg px-3 py-2 text-sm outline-none focus:border-pink-500/60" placeholder="Ex: bastidores da produção" />
                </label>
                <label className="md:col-span-2">
                  <span className="block text-[10px] uppercase text-text-secondary font-bold mb-1">Restrições ou observações opcionais</span>
                  <textarea value={briefing.instructions} onChange={(event) => updateBriefing("instructions", event.target.value)} className="w-full h-28 bg-background border border-outline-border rounded-lg px-3 py-2 text-sm outline-none resize-none focus:border-pink-500/60" placeholder="Ex: não mencionar preço; evitar tom promocional; usar somente fotos reais." />
                </label>
              </div>

              <div className="mt-5 flex justify-end">
                <button type="button" disabled={!briefingReady} onClick={confirmBriefing} className="px-5 py-3 bg-pink-600 hover:bg-pink-500 text-white rounded-xl font-bold text-sm flex items-center gap-2 disabled:opacity-40">
                  Continuar para ideias <ArrowRight className="w-4 h-4" />
                </button>
              </div>
            </section>
            ...
            <aside className="bg-surface-card border border-outline-border p-5 rounded-2xl self-start">
              ...
              {baseStatus.ready ? (
                <div className="mt-4 rounded-xl border border-emerald-500/25 bg-emerald-500/10 p-4">
                  <div className="flex items-center gap-2 text-emerald-300 text-xs font-bold"><Check className="w-4 h-4" />Base Inicial pronta</div>
                  <p className="mt-2 text-[11px] text-text-secondary">Os documentos canônicos estão confirmados e podem ser usados para fundamentar a criação.</p>
                </div>
              ) : (
                <div className="mt-4 rounded-xl border border-amber-500/25 bg-amber-500/10 p-4">
                  <div className="flex items-center gap-2 text-amber-300 text-xs font-bold"><AlertTriangle className="w-4 h-4" />Base ainda não está pronta</div>
              ...
              {!briefingValidation.valid && (
                <div className="mt-3 text-[11px] text-text-secondary">
                  Para continuar, preencha: {briefingValidation.missing.map((field) => ({ objective: "objetivo", format: "formato", channel: "canal" })[field]).join(", ")}.
                </div>
              )}
            </aside>
          </div>
        ...
        {stage === "ideas" && (
          <div className="grid grid-cols-1 xl:grid-cols-[330px_minmax(0,1fr)] gap-5">
            <aside className="bg-surface-card border border-outline-border p-5 rounded-2xl self-start">
              ...
              <button disabled={isGenerating} onClick={() => void handleGenerateIdeas()} className="w-full mt-5 bg-pink-600 hover:bg-pink-500 text-white rounded-xl py-3 font-bold text-sm flex items-center justify-center disabled:opacity-40">
                    ...
                    <article key={`${idea.title}-${index}`} className="border border-outline-border bg-background/35 p-5 rounded-xl">
                      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                        ...
                        <div className="flex flex-wrap gap-2 shrink-0">
                          <button type="button" onClick={() => beginDevelopment({ id: savedIdeaIds[index], title: idea.title, hook: idea.hook, format: idea.format, channel: idea.channel, objective: idea.objective, concept: idea.concept, keyMessage: idea.keyMessage, callToAction: idea.cta })} className="px-3 py-2 rounded-lg bg-pink-500/15 border border-pink-500/25 text-pink-300 text-xs font-bold flex items-center gap-1.5">
                            Desenvolver <ArrowRight className="w-3.5 h-3.5" />
                          </button>
                          <button disabled={isSaving || Boolean(savedIdeaIds[index])} onClick={() => void handleSaveIdea(idea, index)} className="px-3 py-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 text-xs font-bold flex items-center gap-1.5 disabled:opacity-50">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-[350px_minmax(0,1fr)] gap-5">
            <aside className="bg-surface-card border border-outline-border p-5 rounded-2xl self-start">
              ...
              <label className="block mt-5">
                ...
                <select value={sourceIdeaId || ""} onChange={(event) => handleSavedIdeaSelection(event.target.value)} className="w-full bg-background border border-outline-border rounded-lg px-3 py-2 text-sm outline-none">
              ...
              <button disabled={isGenerating || !scriptIdea.trim() || !scriptObjective.trim() || !scriptFormat.trim() || !scriptPlatform.trim()} onClick={() => void handleGenerateScript()} className="w-full mt-5 bg-pink-600 hover:bg-pink-500 text-white rounded-xl py-3 font-bold text-sm flex items-center justify-center disabled:opacity-40">
            ...
            <section className="bg-surface-card border border-outline-border p-5 rounded-2xl min-h-[420px]">
              <div className="border-b border-outline-border pb-3">
                ...
                <p className="text-[11px] text-text-secondary mt-1">Aprovar e salvar é uma decisão de workflow. O conteúdo continua HIPÓTESE no Obsidian até validação factual.</p>
              </div>
                  ...
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                    ...
                    <div className="flex flex-wrap gap-2 shrink-0">
                      <button disabled={isSaving || Boolean(savedScriptId)} onClick={() => void handleSaveScript(generatedScript)} className="px-3 py-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 text-xs font-bold flex items-center gap-1.5 disabled:opacity-50">
                        ...
                        {savedScriptId ? "Aprovado e salvo" : "Aprovar e salvar"}
                      </button>
                      <button disabled={!savedScriptId} onClick={openPlanning} title={!savedScriptId ? "Aprove e salve o conteúdo antes de planejar" : "Escolha a data de publicação"} className="px-3 py-2 rounded-lg bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 text-xs font-bold flex items-center gap-1.5 disabled:opacity-40">
